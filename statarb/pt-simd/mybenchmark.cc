...
double calc_mean(const std::vector<double> &v) {
  ...
  __m256d sum_vec_total =
      _mm256_add_pd(temp, _mm256_permute2f128_pd(temp, temp, 0x1));
...
double calc_stddev(const std::vector<double> &v, double mean) {
  ...
  __m256d sum_vec_total =
      _mm256_add_pd(temp1, _mm256_permute2f128_pd(temp1, temp1, 0x1));

  __m256d temp2 = _mm256_hadd_pd(sq_sum_vec, sq_sum_vec);
  __m256d sq_sum_vec_total =
      _mm256_add_pd(temp2, _mm256_permute2f128_pd(temp2, temp2, 0x1));
...
void pairs_trading_strategy(const std::vector<double> &stock1_prices,
                            const std::vector<double> &stock2_prices,
                            size_t N) {
  // implementation...
  ...
  for (size_t i = N; i < stock1_prices.size(); ++i) {
    ...
    std::vector<double> spread(stock1_prices.begin() + i - N,
                               stock1_prices.begin() + i);
