...
Disruptor::Disruptor(size_t bufferSize,
                     std::vector<EventProcessor *> &processors,
                     std::vector<Producer *> &producers,
                     WaitStrategy *waitStrategy)
    : buffer(std::make_shared<RingBuffer>(bufferSize)),
      sequencer(std::make_shared<Sequencer>()), processors(processors),
      producers(producers), waitStrategy(waitStrategy) {
